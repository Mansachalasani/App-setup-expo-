...
const PatientHomeScreen: React.FC = () => {
  ...
  const getVisitAccent = (visit: Visit): string[] => {
    const t = (visit.visit_type || '').toLowerCase()
    if (t.includes('virtual')) return ['#8E2DE2', '#4A00E0']
    if (t.includes('self')) return ['#4CAF50', '#81C784']
    return ['#2196F3', '#64B5F6']
  }

  const getVisitTypeLabel = (visit: Visit): string => {
    const t = (visit.visit_type || '').toLowerCase()
    if (t.includes('virtual')) return 'Virtual'
    if (t.includes('self')) return 'Self-Recorded'
    return 'In-Person'
  }

  const renderVisitCard = (visit: Visit) => (
    <TouchableOpacity key={visit.id} style={styles.visitCard} activeOpacity={0.9} onPress={() => {}}>
      <LinearGradient colors={getVisitAccent(visit)} style={styles.accentStrip} />
      {/* Visit Header */}
      <View style={styles.visitHeader}>
        ...
        <View style={styles.visitTypePill}>
          <MaterialIcons
            name={(visit as any).field_doctors ? 'medical-services' : (visit.visit_type || '').includes('virtual') ? 'videocam' : 'self-improvement'}
            size={14}
            color="#FFFFFF"
          />
          <Text style={styles.visitTypeText}>{getVisitTypeLabel(visit)}</Text>
        </View>
      </View>
        ...
        >
          <View style={styles.welcomeHeader}>
            <View style={styles.greetingContainer}>
              ...
              <Text style={styles.nameTextLight}>{patient?.name}</Text>
            </View>
            <TouchableOpacity
              style={styles.profileButtonLight}
          ...
          <Text style={styles.welcomeSubTextLight}>Here's your health overview</Text>
        </LinearGradient>

        {/* Quick Stats */}
        <View style={styles.statsContainer}>
          <TouchableOpacity activeOpacity={0.85} onPress={() => router.push('/patient/history')}>
            <LinearGradient colors={["#2196F3", "#64B5F6"]} style={styles.statCardGrad}>
              <View style={styles.statIconLight}>
                <MaterialIcons name="history" size={24} color="#FFFFFF" />
              </View>
              <Text style={styles.statNumberLight}>{recentVisits.length}</Text>
              <Text style={styles.statLabelLight}>Recent Visits</Text>
            </LinearGradient>
          </TouchableOpacity>

          <TouchableOpacity activeOpacity={0.85} onPress={() => router.push('/patient/AddVitals')}>
            <LinearGradient colors={["#4CAF50", "#81C784"]} style={styles.statCardGrad}>
              <View style={styles.statIconLight}>
                <MaterialIcons name="notes" size={24} color="#FFFFFF" />
              </View>
              <Text style={styles.statNumberLight}>Add</Text>
              <Text style={styles.statLabelLight}>Vitals Entry</Text>
            </LinearGradient>
          </TouchableOpacity>

          <TouchableOpacity activeOpacity={0.85} onPress={() => router.push('/patient/profile')}>
            <LinearGradient colors={["#FF9800", "#FFB74D"]} style={styles.statCardGrad}>
              <View style={styles.statIconLight}>
                <MaterialIcons name="person" size={24} color="#FFFFFF" />
              </View>
              <Text style={styles.statNumberLight}>{patient?.age}</Text>
              <Text style={styles.statLabelLight}>Years Old</Text>
            </LinearGradient>
          </TouchableOpacity>
        </View>
          ...
          {recentVisits.length > 0 ? (
            ...
            >
              {recentVisits.map((visit) => (
                <View key={visit.id} style={styles.carouselItem}>
...
const styles = StyleSheet.create({
  ...
  // Added: Gradient welcome variant
  welcomeGradient: {
    borderRadius: 16,
    padding: 16,
    marginBottom: 24,
