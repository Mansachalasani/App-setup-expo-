...
const HealthTrendsComponent: React.FC<HealthTrendsProps> = ({
  ...
  const renderChart = () => {
    ...
    const suffix = (vitalOptions.find(v => v.value === selectedVital)?.unit) || '';

    return (
      <TouchableOpacity activeOpacity={0.9} onPress={() => setExpandVisible(true)} style={styles.chartTapArea}>
        <LineChart
          data={{
            labels: data.labels,
    ...
    return (
      <View style={styles.statsSection}>
        ...
        <View style={styles.statsGrid}>
          <View style={styles.statCard}>
            ...
            <View
              style={[
                styles.statusBadge,
                { backgroundColor: `${latestStatus.color}20` },
              ]}
            >
        ...
        <Text style={styles.statsFooter}>
          Based on {stats.count} reading{stats.count !== 1 ? "s" : ""} • Unit:{" "}
          {vitalInfo?.unit}
        </Text>
      </View>
    );
  ...
  return (
    ...
    >
      ...
      {showHeader && (
        <View style={styles.headerSection}>
          ...
          <Text style={styles.headerSubtitle}>Track vital signs over time</Text>
        </View>
      )}
      ...
      <View style={styles.section}>
        <View style={styles.chartHeader}>
          <MaterialIcons
            name={
              (vitalOptions.find((v) => v.value === selectedVital)
                ?.icon as any) || "trending-up"
            }
            size={24}
            color="#4285F4"
          />
        ...
        <View style={styles.tipCard}>
          ...
          <View style={styles.tipContent}>
            ...
            <Text style={styles.tipText}>
              ...
              Consult your doctor if you notice concerning trends.
            </Text>
          </View>
        </View>
      </View>
    ...
    <Modal visible={expandVisible} animationType="fade" transparent>
        ...
        <View style={styles.modalBody}>
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              translateX.setValue(0); translateX.setOffset(0);
              translateY.setValue(0); translateY.setOffset(0);
          ...
          >
            <PanGestureHandler
              ...
              onHandlerStateChange={(e) => {
                if (e.nativeEvent.state === State.END || e.nativeEvent.state === State.CANCELLED) {
                  offset.current = {
                    x: offset.current.x + e.nativeEvent.translationX,
                    y: offset.current.y + e.nativeEvent.translationY,
                  };
                  translateX.setOffset(offset.current.x); translateX.setValue(0);
                  translateY.setOffset(offset.current.y); translateY.setValue(0);
                }
              }}
            >
              <Animated.View
              ...
              >
                ...
                >
                  {(() => {
                    const d = processVitalData(selectedVital);
                    if (!d) return null;
                    const cfg = {
                      backgroundGradientFrom: '#FFFFFF',
                      backgroundGradientTo: '#FFFFFF',
                      color: (o = 1) => `rgba(26,115,232,${o})`,
                      strokeWidth: 2,
                      decimalPlaces: selectedVital === 'temperature' ? 1 : 0,
                      propsForDots: { r: '3.5', strokeWidth: '2', stroke: '#ffffff' },
                    } as any;
                    const suffix = (vitalOptions.find(v => v.value === selectedVital)?.unit) || '';
                    return (
                      <>
                        <LineChart
                          data={{ labels: d.labels, datasets: d.datasets, legend: selectedVital === 'blood_pressure' ? ['Systolic','Diastolic'] : undefined }}
                          width={screenWidth}
                        ...
                        />
                        {(() => {
                          const s = getVitalStats(selectedVital);
                          if (!s) return null;
                          return (
                            <View style={styles.modalStatsGrid}>
                              <View style={styles.modalStat}><Text style={styles.modalStatValue}>{s.latest}</Text><Text style={styles.modalStatLabel}>Latest</Text></View>
                              <View style={styles.modalStat}><Text style={styles.modalStatValue}>{s.avg}</Text><Text style={styles.modalStatLabel}>Avg</Text></View>
                              <View style={styles.modalStat}><Text style={styles.modalStatValue}>{s.min}</Text><Text style={styles.modalStatLabel}>Min</Text></View>
                              <View style={styles.modalStat}><Text style={styles.modalStatValue}>{s.max}</Text><Text style={styles.modalStatLabel}>Max</Text></View>
                            </View>
                          );
                        })()}
                      </>
